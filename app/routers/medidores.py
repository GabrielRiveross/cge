...
def list_medidores(db: Session = Depends(get_db)):
    rows = (
        db.query(
            ...
            models.Medidor.latitud,
            models.Medidor.longitud,
...
def get_medidor(id: int, db: Session = Depends(get_db)):
    ...
    # agregar cliente_nombre para que lo reciba el schema
    if obj.cliente:
...
def listar_medidores_por_cliente(id_cliente: int, db: Session = Depends(get_db)):
    rows = (
        db.query(
            models.Medidor.id_medidor,
            models.Medidor.codigo_medidor,
    ...
    # mismo patrón que list_medidores
    return [schemas.MedidorOut(**dict(r._mapping)) for r in rows]
