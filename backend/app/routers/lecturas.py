...
def crear_lectura(payload: schemas.LecturaCreate, db: Session = Depends(get_db)):
    ...
    # (Opcional) Chequear duplicados por (id_medidor, anio, mes)
    dup = (
...
def listar_por_medidor(id_medidor: int, db: Session = Depends(get_db)):
    ...
    return (
        db.query(models.Lectura)
        .filter(models.Lectura.id_medidor == id_medidor)
        .order_by(models.Lectura.anio.desc(), models.Lectura.mes.desc())
        .all()
    )
