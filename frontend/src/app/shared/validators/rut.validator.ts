...
function splitRut(raw: string): { num: string; dv: string } | null {
  ...
  if (!/^\d+$/.test(num)) return null; // Solo números

  return { num, dv };
}

// Calcula el dígito verificador aplicando la regla del módulo 11
...
// Validador Angular (RUT persona y empresa en general)
export function rutPersonaValidator(): ValidatorFn {
  return (control: AbstractControl): ValidationErrors | null => {
    const raw = control.value as string;
    if (!raw) return null; // Permite requerido controlarlo desde otro validator
...
// Formatea RUT con puntos y guion: 12345678K → 12.345.678-K
